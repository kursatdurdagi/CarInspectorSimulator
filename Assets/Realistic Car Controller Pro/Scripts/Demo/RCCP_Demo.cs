...
public class RCCP_Demo : RCCP_GenericComponent {
    ...
    public void SpawnPhoton() {
        ...
        // Here we are creating our new vehicle.
#if RCCP_DEMO
        GameObject spawnedGO = PhotonNetwork.Instantiate(RCCP_DemoVehicles_Photon.Instance.vehicles[selectedVehicleIndex].transform.name, lastKnownPos, lastKnownRot);
        spawnedGO.TryGetComponent<RCCP_CarController>(out var spawnedVehicle);
        RCCP.RegisterPlayerVehicle(spawnedVehicle, true, true);
#else
        GameObject spawnedGO = PhotonNetwork.Instantiate(RCCP_Prototype_Photon.Instance.vehicles[selectedVehicleIndex].transform.name, lastKnownPos, lastKnownRot);
        spawnedGO.TryGetComponent<RCCP_CarController>(out var spawnedVehicle);
        RCCP.RegisterPlayerVehicle(spawnedVehicle, true, true);
#endif
