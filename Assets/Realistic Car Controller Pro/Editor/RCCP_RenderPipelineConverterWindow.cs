...
public class RCCP_RenderPipelineConverterWindow : EditorWindow {
    ...
    // Expected package name for importPackageCompleted callback validation.
    ...
    /// <summary>
    /// Opens Unity's built-in Render Pipeline Converter window.
    /// </summary>
        ...
        if (!bypassDefaultShaderCheck) {
            ...
            if (GraphicsSettings.currentRenderPipeline != null) {
                ...
                if (!string.IsNullOrEmpty(builtinPackagePath)) {
                    ...
                    // Store expected package name and subscribe before we import
    ...
    /// <summary>
    /// Called when a package import completes. Only proceeds if it matches an HDRP shader package.
    /// </summary>
    private static void OnHDRPShadersImported(string packageName) {
        ...
        if (!packageName.Contains("HDRPShaders")) {
            return;
        }
